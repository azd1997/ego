...
func NewDataPack(packetSize int) *DataPack {
	return &DataPack{MaxSize:packetSize}
}

func (dp *DataPack) MaxPacketSize() int {
	return dp.MaxSize
}
...
func (dp *DataPack) Unpack(binaryData []byte) (IMessage, error) {
	...
	if dp.MaxSize > 0 && int(msg.DataLen) > dp.MaxSize {
